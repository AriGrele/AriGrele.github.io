...
func _ready():
	...
	commentbox = get_node('/root/Main/View/CanvasLayer/PageCenter/StatsPanel/comments')
...
func dcircle(Start,dim,fill,cols):
		...
		if abs(dim.x*dim.y)>0:
			...
			for k in boxes[key]:
				if str(k).split('_')[0]=='Comment':
					comcount+=1
					draw_string(font,start-Vector2(0,5+comcount*12),boxes[key][k],Color(0,0,0))
